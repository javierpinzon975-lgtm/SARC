...
function toggleRegistrationFields() {
    ...
    const extraFields = document.getElementById('extra-reg-fields');
    const isPaciente = role === 'paciente';

    extraFields.style.display = isPaciente ? 'block' : 'none';

    if (!isPaciente) {
        document.getElementById('reg-dob').required = false;
        document.getElementById('reg-regimen').required = false;
        document.getElementById('reg-celular').required = false;
        document.getElementById('reg-email').required = false;
    } else {
        document.getElementById('reg-dob').required = true;
        document.getElementById('reg-regimen').required = true;
        document.getElementById('reg-celular').required = true;
...
function handleRegister(event) {
    ...
    if (!nombre || !id) {
        showToast("Nombre e identificación son obligatorios.", "danger");
        return;
    }

    if (role === 'paciente' && (!fechaNacimiento || !regimen || !celular || !correo)) {
        showToast("Para registrar un paciente, debe completar los datos adicionales.", "danger");
        return;
    }
    ...
    const nuevoUsuario = {
        nombre,
        id,
        fechaNacimiento: role === 'paciente' ? fechaNacimiento : '',
        regimen: role === 'paciente' ? regimen : '',
        celular: role === 'paciente' ? celular : '',
        correo: role === 'paciente' ? correo : '',
        tipo: role
    };
    ...
    showToast(`Registro de ${role === 'paciente' ? 'paciente' : role === 'medico' ? 'médico' : 'recepcionista'} exitoso. Proceda a iniciar sesión.`, "success");
...
function handleBooking(event) {
    ...
    if (!currentUser?.celular || !currentUser?.correo) {
        showToast("Falta información de contacto del paciente.", "danger");
        return;
    }
...
async function generarPDFParte(cita, parte, edad, medicoNombre, medicoId) {
    ...
    try {
        const logoBase64 = await getBase64ImageFromUrl('png/Logo%20P%26G.png');
